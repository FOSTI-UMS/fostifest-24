...
const UploadFileForm = ({ accept, bucket, onChange, onLoading, color, folder, maxSizeMB, validFileTypes }) => {
  ...
  useEffect(() => {
    setFileUploadColor(newFileUrl ? "main" : color);
  }, [color, newFileUrl]);
  ...
  return (
    <div className="w-full relative flex items-center justify-center flex-col">
            ...
            <>
              {accept !== "image/jpg,image/jpeg,image/png" ? (
                ...
                <div className="relative w-full h-full">
                  <Image src={newFileUrl} alt="Uploaded File" className="rounded-lg h-full w-full hover absolute object-contain" width={1080} height={1080} />
                  <CustomButton
                    as="button"
      ...
      {isDragging && <div className="h-[9999px] w-screen bg-black bg-opacity-40 absolute z-0" />}
    </div>
  );
};
