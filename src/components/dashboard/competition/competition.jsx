...
const Competition = () => {
  ...
  useEffect(() => {
    ...
    const updatedCompetitionList = categories.map((cat) => ({
      ...
      status: competitions.find((c) => c.category === cat.title)?.status || "Belum Bayar",
    }));
          ...
          <div key={index} className="mb-5">
            ...
            <div className="space-y-5 bg-gradient-to-tr from-[#191834] to-[#444ca6] rounded-xl md:p-8 p-5">
              <div className="flex items-center justify-start space-x-3">
                ...
                <p className={`py-2 px-5 rounded-md max-w-fit text-sm cursor-default 
                  ${item.status === "Belum Bayar" ? "bg-red-600" 
                  : item.status === "Menunggu Verifikasi" ? "bg-orange-400" 
                  : item.status === "Sudah Bayar" ? "bg-green-600" 
                  : "bg-gray-500"}`}>
                  {item.status}
                </p>
              </div>
