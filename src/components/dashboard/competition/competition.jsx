...
const Competition = () => {
  return (
    <div className="md:container">
      ...
      <h2 className="text-xl font-semibold md:mb-0 mb-3">Partisipasi dalam Kompetisi</h2>

      <div className="flex lg:flex-row flex-col lg:space-x-3 lg:space-y-0 space-y-3 justify-start items-start md:mb-0 mb-10">
        {competitions.map((card, index) => (
          <CardContainer key={index} className="inter-var">
            <CardBody className="h-auto lg:w-[250px] bg-gradient-to-tr from-[#191834] to-[#444ca6] transition-all duration-300 bg-gray-50 relative group/card dark:hover:shadow-2xl dark:hover:shadow-main-primary/[0.8] border-black/[0.1] md:rounded-xl rounded-lg p-6 border">
              <CardItem translateZ="100" className="mt-2 mb-3">
                <Image src={card.imageSrc} height="1000" width="1000" className="h-16 w-full object-contain rounded-xl" alt={card.title} />
              </CardItem>
              ...
              {card.isRegistered && (
               <CardItem translateZ="60" className="text-neutral-500 text-sm max-w-sm mt-2 dark:text-neutral-300">
               <HoverBorderGradient
                 className="px-7 text-main-primary font-semibold "     
                 containerClassName="cursor-default justify-center items-center max-w-fit flex h-10 mt-5 border main-shadow-hover relative rounded-xl"
               >
      ...
      {competitions
        .filter((card) => card.isRegistered)
        .map((card, index) => (
          <div key={index} className="mb-5">
            <h3 className="font-semibold text-xl my-3">{card.title}</h3>
            <div className="space-y-5 bg-gradient-to-tr from-[#191834] to-[#444ca6] rounded-xl md:p-8 p-5">
              <div className="flex items-center justify-start space-x-3">
                <h2 className="font-medium">Status : </h2>
                <p className={`py-2 px-5 rounded-md max-w-fit text-sm cursor-default ${card.status === "Belum Bayar" ? "bg-red-600" : card.status === "Sudah Bayar" ? "bg-green-600" : "bg-yellow-600"}`}>{card.status}</p>
              </div>
