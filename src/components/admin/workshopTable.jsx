          ...
          <tbody>
            {filteredWorkshop.map((item, index) => (
              <tr className="hover:bg-gray-700" key={item.userId}>
                ...
                <td className={`text-center border border-white ${handleStatusBg(item.workshopStatus)}`}>{item.workshopPayment ? item.workshopStatus : PaymentStatusConstant.notPaid}</td>
                <td className="border text-center border-white">
                  {item.workshopStatus === PaymentStatusConstant.paid && <div className="bg-green-500 py-4 ">Terverifikasi</div>}
