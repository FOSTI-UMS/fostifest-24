...
export default function UiUxDesignTable() {
  const { loading, uiUxDesign } = useAdmin();
  ...
  const filteredUiUxDesign = uiUxDesign.filter((item) => item.userEmail.toLowerCase().includes(searchEmail.toLowerCase()));
  ...
  const exportToExcel = () => {
    try {
      ...
      XLSX.utils.book_append_sheet(workbook, worksheet, "UI_UX Design");

      XLSX.writeFile(workbook, "UI_UX_Design.xlsx");
    } catch (error) {
  ...
  return (
    <>
      <h2 className="mb-3 text-xl font-semibold">{CompetitionCategoriesConstant.ud}</h2>
      ...
      {!loading && filteredUiUxDesign.length <= 0 && <p className="text-lg font-medium text-center mt-4 text-gray-500">{searchEmail ? "Peserta tidak ditemukan" : "Belum ada peserta"}</p>}
      {!loading && filteredUiUxDesign.length > 0 && (
