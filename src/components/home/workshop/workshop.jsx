...
const WorkShopSection = () => {
  const { eventStart, registrationEnd, now, loading, session, sectionRefs } = useUser();
  ...
  return (
    <section ref={sectionRefs.workshop} id="workshop" className="md:container container-none my-[60px] flex flex-col lg:flex-row lg:pe-8 mb-5 justify-center items-center">
      ...
      <div className="flex-col justify-center items-center lg:w-1/2 w-full rounded-2xl md:p-8 p-5 ">
        ...
        {!loading && !session && now >= eventStart && now <= registrationEnd && <CustomButton href={"/register"} text={"Join Workshop"} />}
      </div>
    </section>
  );
};
